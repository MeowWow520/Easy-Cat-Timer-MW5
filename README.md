...
1. 新增：窗口置顶——点击标题栏上的[置顶]开关，可以让窗口始终显示在其它窗口之上；退出程序时会记住这个设置
2. 优化：当主窗口置顶的时候，右下角的通知窗口也会一起置顶

<br/>

**v1.1.1.0（2026.09.18）：**

1. 新增：番茄钟——在设置界面里勾选[番茄钟]，就可以使用番茄工作法
   （默认：工作25分钟 + 短休息5分钟 + 长休息15分钟，每4轮长休息一次；这4项都可以在番茄钟界面里自己调整）
2. 新增：一个阶段结束之后，会自动进入下一个阶段；也可以点标题栏上的[跳过]按钮，直接进入下一个阶段
   （[自动进入下一阶段]可以在设置界面里关掉，关掉之后就要自己点[开始]）
3. 新增：番茄钟运行的时候，时钟上方会显示当前是哪个阶段（工作/短休息/长休息）、以及第几轮
4. 优化：点黑猫[重置]的时候，番茄钟也会一起复位
